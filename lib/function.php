...
{
    $sql = "SELECT tbl_tour.id AS tour_id, tbl_tour.id,tbl_tour.name, tbl_tour.note, tbl_tour.price, tbl_tour.day, tbl_tour.img_id, tbl_tour.status, tbl_media.name_file FROM tbl_tour LEFT JOIN tbl_media ON tbl_tour.img_id = tbl_media.id WHERE tbl_tour.status = 'tour_hot';";
    $tours = getDaTa($sql);
    return $tours;
}
...
function getHotBlog()
{
    $sql = "SELECT tbl_blog.id AS blog_id, tbl_blog.title, tbl_blog.content, tbl_blog.status,  tbl_blog.img_id,tbl_blog.count_like, tbl_media.name_file FROM tbl_blog LEFT JOIN tbl_media ON tbl_blog.img_id = tbl_media.id WHERE tbl_blog.status = 'blog_hot'";
    $blogs = getData($sql);
    return $blogs;
}
